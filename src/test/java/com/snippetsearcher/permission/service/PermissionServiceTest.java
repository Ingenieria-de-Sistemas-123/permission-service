...
class PermissionServiceTest {
  ...
  void create_throwsConflict_when_permissionAlreadyExists_forSnippetAndUser() {
    ...
    when(repo.findBySnippetIdAndUserId(snippetId, userId))
  ...
  void create_throwsConflict_when_typeOwner_and_ownerAlreadyExistsForSnippet() {
    ...
    var ex =
        assertThrows(
            ConflictException.class, () -> service.create(snippetId, userId, PermissionType.OWNER));
  ...
  void create_saves_and_returns_permission_when_valid() {
    ...
    when(repo.save(any(Permission.class))).thenAnswer(inv -> inv.getArgument(0, Permission.class));
  ...
  void getAuthor_returns_owner_permission() {
    ...
    when(repo.findBySnippetIdAndType(snippetId, PermissionType.OWNER))
